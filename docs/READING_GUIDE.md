...
このコードベースを読むときの手順と道具です。
**何が正しいか**は [ARCHITECTURE.md](../ARCHITECTURE.md) に、
**どう見つけるか**はこの文書に書いてあります。

---

## 0. 最初に覚える3つの入口

読み始める場所は、知りたいことによって変わります。
...
**迷ったら `AppRouter.tsx` から読んでください。** アプリの全画面が1ファイルに並んでいます。

---

## 1. import 文を見ればレイヤーが分かる

このプロジェクトの import はすべて `@/` から始まります（相対パスの `../../` は同じ機能の中だけ）。
つまりファイルの先頭を見るだけで、**そのファイルがどのレイヤーに依存しているか**が分かります。
...
逆に言うと、**import 文だけで「このファイルは何をする階層のものか」が判別できます。**
判別できないファイルがあれば、それは置き場所が間違っているサインです。

---

## 2. 「データがどこから来たか」を追う：4つの層をたどる

画面に出ている値は、必ずこの順で流れています。
...
**逆方向（API から画面へ）に追いたいときは、パス文字列で grep します。**
...
TanStack Query は **queryKey が同じものを同じデータとして扱います。**
つまり「更新した側」と「表示している側」を繋いでいるのは queryKey だけです。
...
「カートに追加したらヘッダの数字が増える」の仕組みは、この grep 結果を並べるだけで見えます。

> **道具**: 画面左下の React Query Devtools を開くと、いま何のキーにどんなデータが入っていて、
> どれが `fresh` / `stale` / `fetching` なのかが目で見えます。
> ボタンを押す前後で見比べると、invalidate が何をしているか一目で分かります。

---

## 4. 「この state は誰が持っているか」を見分ける

このコードベースでは、状態の持ち主が3種類しかありません。**まずどれかを判定してください。**
...
`src/pages/` の中に `useState` はほとんど出てきません（検索キーワードくらい）。
**「ページに state が無い」こと自体が、この設計の主張です。**

なお、トークンだけは3種類のどれでもありません。どこにあるかは
`src/shared/api/useAuthToken.ts` を読んでください。

---

## 5. 「この値の型はどこで変わったか」を追う

同じ値がレイヤーをまたぐと型が変わります。変わる場所は決まっています。
...
`ProductResponse` が出てくるのが `entities/product/api/` だけであることを確認してください。
**それが「バックエンドの形を1箇所に閉じ込めた」ことの証明です。**

---

## 6. 「このエラーはどこから来たか」を追う

画面にエラーが出たら、まず **kind** を見ます。
...
`try / catch` を grep しても、画面側には1つも見つかりません。
エラーの形を整える処理が `src/shared/api/httpClient.ts` の interceptor に集約されているためです。

```bash
grep -rn "catch" src/pages src/features   # → 出てこない
```

**「無いこと」を確認するのも読み方のひとつです。**

---

## 7. テストを仕様書として読む

「この機能は何を保証しているのか」が分からなくなったら、テストを読んでください。
日本語で書いてあります。
...
「なぜこのファイルはここにあるのか」が分からないときは、**動かしてみると答えが出ます。**
...
3〜5 を繰り返します。**同時に2箇所直さないでください。** 何が効いたのか分からなくなります。
...
| コメントの「なぜ」を読まずに実装だけ見る | このコードベースは「なぜ」がコメントに書いてある。実装より先に読む |

---

## 次にやること

読み方が分かったら、**壊れたコードを直してみてください。**
